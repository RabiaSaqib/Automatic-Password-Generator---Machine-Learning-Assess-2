...
Top.pack(side=TOP)
Top.config(bg = '#0b0233')
Form = Frame(gui, width=width, background="#0b0233",)
Form.pack(side=TOP)
Bot = Frame(gui, width=width)
Bot.pack(side=BOTTOM)

#Label
label_password = Label(Form, font=('Segoe UI', 18), text="Password",foreground="white", background="#0b0233", bd=10)
...
label_instructions = Label(Bot, width=width, font=('Segoe UI', 12, 'bold'), text="Password Generated to your Clipboard!", foreground="black", background="red", bd=1, relief=SOLID)
...
Button(Top, text = 'COPY The Password', foreground="red", background="#1A1A1A", command = Copy_password).pack(pady=5)
